...
public struct CountedChunk: Hashable, Comparable {
    ...
    public init(_ chunk: Chunk, count: Int = 0) {
        value = chunk
        self.count = count
    }

    /// Use only the creation date in determining less or greater than
    public static func < (lhs: CountedChunk, rhs: CountedChunk) -> Bool {
        lhs.count < rhs.count
    }
    ...
    public func incremented() -> CountedChunk {
